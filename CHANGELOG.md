...
本次更新引入对话记忆机制和用户偏好系统，显著提升AI建议的连贯性和个性化程度。

#### 新增功能

**1. 对话记忆机制**

- 保存最近20条消息（user/assistant/tool消息）
- 战斗开始/结束时自动重置记忆
- 消息结构验证和自动修复

**2. 工具结果缓存**

- `STABLE`类型工具首次执行后缓存（牌组、遗物、战术知识）
- `REALTIME`类型工具每次执行（手牌、血量、敌人）
- 战斗结束时缓存失效

**3. 用户偏好系统**

用户可通过对话积累偏好，AI在后续建议中参考：

| 新增文件 | 用途 |
|----------|------|
| `memory/PreferenceManager.java` | 偏好管理器，内存存储+异步持久化 |
| `model/UserPreference.java` | 偏好模型（语义化描述） |
| `tool/GetUserPreferencesTool.java` | 获取历史偏好 |
| `tool/SaveUserPreferenceTool.java` | 保存新偏好 |

**4. 通用场景对话**

支持在任意场景与AI对话，工具自动过滤可用性：

| 场景 | 可用工具 |
|------|----------|
| `battle` | 手牌、敌人、玩家状态、牌组、遗物... |
| `reward` | 卡牌奖励、牌组、遗物、偏好... |
| `general` | 牌组、遗物、偏好、战术知识 |

**5. Mod HTTP Server**

新增Mod端HTTP服务器（端口17533），支持Overlay主动触发分析：

| 端点 | 说明 |
|------|------|
| `POST /trigger-analysis` | 触发AI分析 |
| `GET /status` | 检查Mod运行状态 |

#### 工具信息类型

| 类型 | 说明 | 工具示例 |
|------|------|----------|
| `REALTIME` | 每回合变化，需每次调用 | get_hand_cards, get_player_state, get_enemies |
| `STABLE` | 战斗内不变，可缓存 | get_deck, get_relics, get_tactical_knowledge |
| `ON_DEMAND` | 按需调用 | get_potions, get_card_rewards, get_user_preferences |
...
│  ┌─────────────────────────────────────────────────────┐   │
│  │  STS AI Advisor Mod                                  │   │
│  │  - GameAgent: 单Agent处理所有场景，带对话记忆         │   │
│  │  - ToolRegistry: 工具注册中心，支持缓存              │   │
│  │  - PreferenceManager: 用户偏好管理                   │   │
│  │  - ModHttpServer: 端口17533，接收Overlay触发         │   │
│  └─────────────────────────────────────────────────────┘   │
└─────────────────────────────────────────────────────────────┘
                              │
                              │ HTTP (双向通信)
                              │ Mod: 17533, Overlay: 17532
                              ▼
┌─────────────────────────────────────────────────────────────┐
